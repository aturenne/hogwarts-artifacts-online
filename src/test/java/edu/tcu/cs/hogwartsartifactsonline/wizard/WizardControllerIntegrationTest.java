...
class WizardControllerIntegrationTest {
    ...
    void testAddWizardErrorWithInvalidInput() throws Exception {
        ...
        this.mockMvc.perform(post(this.baseUrl + "/wizards").contentType(MediaType.APPLICATION_JSON).header(HttpHeaders.AUTHORIZATION, this.token).content(json).accept(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.flag").value(false))
